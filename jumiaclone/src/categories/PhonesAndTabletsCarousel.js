...
function PhonesAndTabletsCarousel() {
  const [error, setError] = useState(null);
  const [phonesAndTablets, setPhonesAndTablet] = useState([]);
    ...
    const fetchData = async () => {
      try {
        ...
        const productsResponse = await axios.get(
          'https://jumia-clone-bra6.onrender.com/api/products/phonesAndTablets',
          {
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
        ...
        setPhonesAndTablet(productsResponse.data);
      } catch (error) {
  ...
  const addToCart = async (productId) => {
    try {
      ...
      setPhonesAndTablet((prevProducts) =>
        ...
        <div className='product-title'>
          {phonesAndTablets.length === 0 && (
            <>
              ...
              Phones And Tablets category gets displayed here.
            </>
          )}
          {phonesAndTablets.length > 0 && <>Limited Stock Deals</>}
        </div>

        <Carousel responsive={responsive}>
          {phonesAndTablets.map((item) => (
...
export default PhonesAndTabletsCarousel;
