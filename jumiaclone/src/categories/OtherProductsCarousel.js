    ...
    const fetchData = async () => {
      try {
        ...
        const productsResponse = await axios.get(
          'https://jumia-clone-bra6.onrender.com/api/products/others',
          {
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
        ...
        setOthers(productsResponse.data);
      } catch (error) {
        ...
        <div className='product-title'>
          {others.length === 0 && (
            <>
              ...
              Others category gets displayed here.
            </>
          )}
          {others.length > 0 && <>Top Deals</>}
        </div>
